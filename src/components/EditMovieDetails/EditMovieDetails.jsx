...
function EditMovieDetails() {
    ...
    useEffect(() => {
        ...
        //there is a movie and its the movie matching the current page
        if (movie != undefined && movie.id == params.id) {
            // dispatch({
    ...
    // display to the dom if there is a movie and it matches the page to load
    if (movie != undefined && movie.id == params.id) {
        return (
            <>
                <CssBaseline />
        ...
        // either current movie is not selected or its not matching the params id
        return (
            <h1>Searching....</h1>
        );
    }
